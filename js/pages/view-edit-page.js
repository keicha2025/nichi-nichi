    ...
    <section class="space-y-6 py-4 animate-in fade-in pb-24">
        <div class="bg-white p-6 rounded-[2.5rem] muji-shadow border border-bdr-subtle space-y-6">
            ...
            <div class="text-center py-2">
                ...
                <div class="text-5xl font-light text-txt-primary">
            ...
            <div class="space-y-5">
                ...
                <div class="flex items-center justify-between px-2 py-2 border-b border-bdr-subtle">
                    ...
                    <div class="text-sm text-txt-primary">{{ form.spendDate.replace('T', ' ') }}</div>
                </div>
                ...
                <div v-if="form.type !== '收款'" class="space-y-2 px-2">
                    ...
                    <div class="flex items-center space-x-2 text-sm text-txt-primary">
                ...
                <div class="px-2 space-y-4">
                    <div class="space-y-1">
                        ...
                        <div class="text-sm text-txt-primary">{{ form.name }}</div>
                    </div>
                    
                    <!-- 5. [補回] 支付方式 -->
                    <div class="space-y-1">
                        <label class="text-[10px] text-txt-secondary uppercase font-medium">支付方式</label>
                        <div class="text-sm text-txt-primary">{{ getPaymentName(form.paymentMethod) }}</div>
                    </div>

                    <div class="space-y-1">
                        <label class="text-[10px] text-txt-secondary uppercase font-medium">備註</label>
                        <div class="text-xs text-txt-secondary whitespace-pre-wrap">{{ form.note || '無備註' }}</div>
                    </div>
                </div>
                ...
                <div v-if="form.type === '支出'" class="pt-4 border-t border-bdr-subtle space-y-4">
                    <div class="flex items-center justify-between px-2">
                        ...
                        <div class="text-xs text-txt-secondary">{{ form.isSplit ? '有' : '無' }}</div>
                    </div>
                    <div v-if="form.isSplit" class="bg-bg-subtle p-6 rounded-3xl space-y-6 mx-2">
                        <div class="text-xs text-txt-primary">{{ form.friendName }}</div>
                        ...
                        <div class="flex items-center justify-between border-t border-bdr-subtle pt-3">
                            ...
                            <div class="text-[10px] text-txt-secondary">{{ form.isAlreadyPaid ? '是' : '否' }}</div>
                        </div>
                    </div>
                </div>

                <!-- 7. 旅行計畫模式 -->
                <div v-if="currentProjectName" class="px-2 pt-2 border-t border-bdr-subtle">
