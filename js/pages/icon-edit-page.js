...
export const IconEditPage = {
    ...
    <section class="animate-in fade-in pb-20">
        <div class="bg-white p-6 rounded-[2.5rem] muji-shadow border border-bdr-subtle flex flex-col min-h-[60vh] relative mt-2">
            ...
            <div class="flex justify-between items-center px-1 border-b border-bdr-subtle pb-5 shrink-0">
                <span class="text-[10px] text-txt-secondary uppercase tracking-[0.3em] font-medium">選取圖示</span>
                <button @click="$emit('cancel')" class="text-[10px] text-txt-muted uppercase tracking-widest hover:text-txt-secondary transition-colors">取消</button>
            </div>
            ...
            <div class="flex-1 flex flex-col pt-6 min-h-0">
                ...
                <div v-if="context" class="mb-4 px-2 shrink-0">
                    <div class="flex items-center space-x-4 bg-bg-subtle p-4 rounded-[2rem] border border-bdr-subtle/50">
                        <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-[var(--action-primary-bg)] shadow-sm">
                            <span class="material-symbols-rounded text-2xl">{{ selectedIcon || 'help' }}</span>
                        </div>
                        <div class="flex flex-col flex-1">
                            <span class="text-[10px] text-txt-secondary uppercase tracking-widest font-medium">名稱</span>
                            <input type="text" v-model="localName" class="text-sm font-bold text-txt-primary bg-transparent outline-none w-full border-b border-transparent focus:border-bdr-default transition-colors py-0.5" placeholder="輸入名稱">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto subtle-scrollbar overscroll-contain px-1 max-h-[220px]">
                    <div v-for="(group, name) in iconGroups" :key="name" class="mb-8 last:mb-0">
                        <h4 class="text-[9px] text-txt-muted font-medium mb-4 uppercase tracking-[0.2em] px-1">{{ name }}</h4>
                        <div class="grid grid-cols-4 gap-3">
                            <button v-for="icon in group" :key="icon" 
                                @click="selectIcon(icon)"
                                :class="selectedIcon === icon ? 'bg-[var(--action-primary-bg)] text-white shadow-lg scale-105' : 'bg-bg-subtle text-txt-secondary'"
                                class="aspect-square rounded-2xl flex items-center justify-center hover:bg-[var(--action-primary-bg)] hover:text-white active:scale-95 transition-all">
            ...
            <div class="pt-6 shrink-0">
                <button @click="confirm" class="w-full bg-[var(--action-primary-bg)] text-white py-5 rounded-2xl text-[10px] font-medium tracking-[0.4em] uppercase shadow-lg active:scale-95 transition-all">
                    儲存
                </button>
            </div>
        </div>
