    ...
    <section class="space-y-6 py-4 animate-in fade-in pb-10">
        ...
        <div class="bg-white p-6 rounded-[2rem] muji-shadow border border-bdr-subtle space-y-6">
            ...
            <div class="text-center space-y-1">
                <h1 class="text-xl font-light text-txt-primary tracking-widest">
                    {{ config.user_name || '使用者' }}
                </h1>
                <p v-if="dateRangeText" class="text-[10px] text-txt-secondary tracking-wider font-english">
                    {{ dateRangeText }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div @click="$emit('switch-tab', 'history')" class="bg-bg-subtle p-4 rounded-2xl active:scale-95 transition-all cursor-pointer flex flex-col items-center justify-center space-y-2 h-28 border border-bdr-subtle">
                    <span class="material-symbols-rounded text-txt-secondary text-2xl">list_alt</span>
                    <span class="text-sm text-txt-primary font-medium tracking-widest">交易明細</span>
                </div>
                <div @click="$emit('switch-tab', 'stats')" class="bg-bg-subtle p-4 rounded-2xl active:scale-95 transition-all cursor-pointer flex flex-col items-center justify-center space-y-2 h-28 border border-bdr-subtle">
                    <span class="material-symbols-rounded text-txt-secondary text-2xl">pie_chart</span>
                    <span class="text-sm text-txt-primary font-medium tracking-widest">統計分析</span>
                </div>
            </div>
        </div>
        
        <!-- Block 2: CTA to Guest Mode -->
        <div @click="enterGuestMode" class="bg-white p-6 rounded-[2rem] muji-shadow border border-bdr-subtle active:scale-95 transition-all cursor-pointer flex items-center justify-between">
            <p class="text-sm text-txt-primary font-medium tracking-widest">打造你的專屬帳本</p>
            <span class="material-symbols-rounded text-txt-muted">arrow_forward</span>
        </div>

        <!-- Block 3: Collection Section -->
        <div v-if="shouldShowCollection" class="space-y-4 pt-2">
            <h2 class="text-[10px] text-txt-muted font-medium tracking-[0.2em] ml-2 font-english">COLLECTION</h2>
            
            <div class="bg-white p-5 rounded-[2rem] muji-shadow border border-bdr-subtle space-y-3">
                <!-- FX Rate -->
                <div v-if="shouldShowFxRate" class="flex justify-between items-center border-b border-bdr-subtle pb-3 last:border-0 last:pb-0">
                    <span class="text-xs text-txt-secondary tracking-wider">匯率設定 (JPY/TWD)</span>
                    <span class="text-sm font-light text-txt-primary tracking-wider font-english">{{ Number(config.fx_rate || 0.22) }}</span>
                </div>
                ...
                <div v-if="projects && projects.length > 0" class="space-y-2 last:border-0 last:pb-0">
                    <p class="text-[10px] text-txt-muted uppercase tracking-widest">記帳專案</p>
                    <div class="space-y-2">
                        <div v-for="proj in projects" :key="proj.id" class="flex justify-between items-center bg-bg-subtle rounded-xl p-3 border border-bdr-subtle">
                            <span class="text-xs text-txt-primary tracking-wider">{{ proj.name }}</span>
                            <span class="text-[10px] text-txt-secondary" v-if="proj.status === 'Archived'">已封存</span>
                        </div>
                    </div>
                </div>
            </div>
