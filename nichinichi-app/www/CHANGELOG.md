...
## [2026-04-04T10:19:00+09:00] Bug Fix: Edit Page Payer & Split Logic

### Summary of changes
Resolved several critical bugs in the Edit Page related to data binding and split mode logic to ensure consistency with the transaction history.

### Technical details of implementation
- **Payer Population Fix**: Improved the payer button highlighting logic to handle both ID matching and name matching (fallback) for legacy or imported data.
- **Split Participant Display**: Fixed a template typo where `visibleFriends` was incorrectly used instead of `displayFriends`, restoring the list of split participants.
- **Split Member Initialization**: Refactored the `watch` handler to filter out "我" (Me) from the `selectedFriends` array when parsing from a stored string, preventing UI state contamination.
- **Split Cancellation Logic**: Updated `prepareAndSubmit` to correctly reset `personalShare` to the full amount and clear `debtAmount`/`friendName` when the split mode is toggled off.

### Affected files or modules
- `/js/pages/edit-page.js`: Fixed data binding, template variables, and submission logic.

### Potential side effects or migration notes
- Transactions that were previously stuck in an inconsistent split state can now be corrected by toggling the split switch off and saving.

*修復編輯錯誤：解決了編輯頁面無法正確顯示付款人與分帳好友的問題，並修正了取消分帳時金額未能恢復為總額的邏輯缺陷。*
...
### Refined Edit Page & Data Binding Fixes
- **Robust Identification**: Enhanced `isMeMatch` to support `'我'`, `'Me'`, and UID, ensuring the "Me" button highlights correctly regardless of storage format.
- **Immediate UI Feedback**: Added a watcher for `form.isSplit` that immediately resets `personalShare` to the total amount when splitting is toggled off, providing instant visual confirmation.
- **Resilient Parsing**: Updated the participant parsing logic in the `form.row` watcher to use a regular expression (`/,\s*/`), making it resilient to inconsistent spacing in stored friend name strings.
- **Cleanup Persistence**: Ensured `debtAmount` and `friendName` are explicitly cleared in both the real-time watcher and the final submission logic to prevent stale split data from lingering.

本次修正強化了「我」與好友的比對邏輯，新增了切換分帳時的即時金額重置，並提升了參與者名單解析的相容性。
...
### Technical details of implementation
- **Robust Field Highlighting**: Implemented `isMeMatch` and `isFriendMatch` comparison helpers in `EditPage` to handle IDs, UIDs, and legacy names, ensuring reliable button highlighting and data binding in Edit mode.
- **Participant Persistence**: Enhanced `displayFriends` to ensure that any friend involved in a transaction (as payer, recipient, or split participant) is visible in the UI even if they are hidden in the main list.
- **Split Mode Logic Refinement**: 
    - Refactored `prepareAndSubmit` to correctly reset `personalShare` and clear `debtAmount`/`friendName` when a split is cancelled.
    - Improved `watch` handler to robustly initialize `selectedFriends` while correctly filtering out personal identity markers (ID/UID/Name).

### Affected files or modules
- `/js/pages/edit-page.js`: Implemented robust comparison logic and fixed split state synchronization.

### Potential side effects or migration notes
- Legacy transactions with inconsistent payer names will now correctly highlight the corresponding friend button if the name match is found.

*本次更新強化了編輯模式的人員比對機制與分帳邏輯，解決了付款人高亮失敗、隱藏好友消失以及取消分帳後金額未重置的問題。*
