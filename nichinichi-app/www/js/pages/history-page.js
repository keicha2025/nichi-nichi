...
export const HistoryPage = {
        ...
        <div v-show="isSelectionMode" class="flex items-center justify-between px-4 py-3 bg-white rounded-2xl border border-bdr-outline animate-in slide-in-from-top-4 duration-300">
            ...
            <div class="flex items-center space-x-1">
                <button @click="toggleSelectAll" class="text-[10px] text-txt-secondary tracking-widest hover:text-txt-primary transition-colors uppercase px-2">
                    {{ (selectedIds.length === transactions.length && transactions.length > 0) ? '取消全選' : '全選' }}
                </button>
                <template v-if="selectedIds.length > 0">
                    <div class="w-px h-3 bg-bdr-outline"></div>
                    <button @click="deleteSelectedItems" class="flex items-center space-x-1 text-txt-secondary hover:text-txt-primary transition-all active:scale-95 px-2">
                ...
                <div class="flex items-center space-x-4 flex-1 min-w-0 mr-2">
                    ...
                    <div class="flex flex-col min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-x-2 mt-0.5 text-[9px]">
                            ...
                            <span v-if="getFriendName(item.payer) !== '我' && item.type === '支出'" class="bg-bg-subtle text-txt-secondary px-1.5 rounded whitespace-nowrap">{{ getFriendName(item.payer) }} 付款</span>
    ...
    `,
    props: ['transactions', 'categories', 'paymentMethods', 'projects', 'fxRate', 'friends', 'currentUser'],
    ...
    methods: {
        ...
        getFriendName(idOrName) {
            ...
            // Fallback for long UI-unfriendly IDs
