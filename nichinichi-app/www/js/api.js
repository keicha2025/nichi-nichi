import {
    db, auth, googleProvider, signInWithPopup, signInWithCredential, signOut, onAuthStateChanged, GoogleAuthProvider,
    ...
    reauthenticateWithPopup, reauthenticateWithCredential, serverTimestamp
...
export const API = {
    ...
    async reauthenticate() {
        try {
            ...
            if (window.Capacitor && window.Capacitor.isNativePlatform()) {
                ...
                const result = await reauthenticateWithCredential(auth.currentUser, credential); // Fixed reauthenticateWithCredential call
                return result.user;
            } else {
    ...
    async logout() {
        if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            ...
            } catch (e) { console.error("Native Logout Error", e); }
        }
        await signOut(auth);
    },
    ...
    async syncWidget(transactions) {
        ...
        try {
            ...
            const calcTotal = (currency) => transactions.reduce((acc, t) => {
                // Check if it's today's expense in the target currency
                const isToday = t.spendDate && t.spendDate.startsWith(localISODate);
                const isTargetCurrency = (t.originalCurrency === currency || t.currency === currency);

                if (t.type === '支出' && isToday && isTargetCurrency) {
                    let amt = 0;
                    // Support '我', 'Me', null, or current UID
                    const isMe = t.payer === '我' || t.payer === 'Me' || !t.payer || (currentUid && t.payer === currentUid);

                    if (isMe) {
                        amt = Number(t.amount);
                    } else {
                        amt = Number(t.personalShare) || 0;
                    }
                    return acc + amt;
                }
                return acc;
            ...
            const jpyTotal = calcTotal('JPY');
            const twdTotal = calcTotal('TWD');

            const jpyDisplay = `¥ ${Math.round(jpyTotal).toLocaleString()}`;
            const twdDisplay = `$ ${Math.round(twdTotal).toLocaleString()}`;

            const { Preferences } = window.Capacitor.Plugins;
            if (Preferences) {
                await Preferences.set({ key: 'today_total_jpy', value: jpyDisplay });
                await Preferences.set({ key: 'today_total_twd', value: twdDisplay });
            }

            const { WidgetUpdate } = window.Capacitor.Plugins;
            if (WidgetUpdate) {
