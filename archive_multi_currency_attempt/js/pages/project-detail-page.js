...
export const ProjectDetailPage = {
            ...
            <div v-if="!isEditing" class="flex-1 flex flex-col justify-center space-y-8">
                ...
                <div class="grid grid-cols-2 gap-4 bg-gray-50 p-6 rounded-3xl relative">
                     <div class="text-center">
                         ...
                         <p class="text-xl font-light text-gray-700">{{ formatCurrency(stats.total, baseCurrency) }}</p>
                     </div>
                     <div class="text-center border-l border-gray-200">
                         <p class="text-[9px] text-gray-400 uppercase tracking-widest mb-1 font-medium">平均日花費</p>
                         <p class="text-xl font-light text-gray-700">{{ formatCurrency(stats.daily, baseCurrency) }}</p>
                     </div>
                </div>
    ...
    `,
    props: ['project', 'transactions', 'fxRate', 'effectiveRates'],
    inject: ['dialog'],
    setup() {
        const { inject, computed } = window.Vue;
        const baseCurrency = inject('baseCurrency');
        return { baseCurrency, formatCurrency };
    },
    data() {
        return {
            isEditing: false,
    ...
    computed: {
        stats() {
            ...
            const txs = (this.transactions || []).filter(t => t && String(t.projectId) === String(this.project.id) && t.type === '支出');
            ...
            txs.forEach(t => {
                ...
                // Convert to Base
                total += convert(amt, currency, this.baseCurrency, rates);
            });
