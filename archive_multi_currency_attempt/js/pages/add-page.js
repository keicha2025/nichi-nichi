...
export const AddPage = {
    ...
    <section class="space-y-6 py-4 animate-in fade-in">
        <div class="bg-white p-6 rounded-[2.5rem] muji-shadow space-y-6">
            ...
            <!-- 2. 金額 -->
            <div class="text-center py-6 border-b border-gray-50 flex flex-col items-center justify-center">
                <div class="flex items-center justify-center space-x-3">
                    <!-- Conditional Selector: <= 3 use Pills, > 3 use Dropdown -->
                    <div v-if="activeCurrencyCodes.length <= 3" class="flex bg-gray-100 rounded-xl p-1 relative h-9">
                        <button v-for="code in activeCurrencyCodes" :key="code"
                                @click="form.currency = code"
                                :class="form.currency === code ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-400'"
                                class="px-3 py-1.5 text-[10px] tracking-widest rounded-lg transition-all font-medium min-w-[3rem]">
                            {{ code }}
                        </button>
                    </div>
                    <currency-select v-else v-model="form.currency" :options="activeCurrencyOptions"></currency-select>
                ...
                <!-- Preview Amount (if different from base) -->
                <div v-if="showPreview" class="mt-2 text-xs text-gray-400 font-light flex items-center space-x-1 animate-in fade-in">
                    <span class="material-symbols-rounded text-[10px]">swap_horiz</span>
            ...
            <div class="space-y-5">
                ...
                <div v-if="form.type === '支出'" class="space-y-2">
                    ...
                    <div class="flex flex-wrap gap-2 px-2">
                        <button @click="form.payer = (systemConfig && systemConfig.user_name) ? systemConfig.user_name : '我'" 
                                :class="(form.payer === '我' || (systemConfig && form.payer === systemConfig.user_name)) ? 'bg-[#4A4A4A] text-white' : 'bg-gray-50 text-gray-400'" 
                                class="px-4 py-1.5 rounded-full text-[10px]">我</button>
    ...
    `,
    props: ['form', 'categories', 'friends', 'loading', 'paymentMethods', 'projects', 'effectiveRates', 'systemConfig'],
    inject: ['dialog'],
    setup(props) {
        ...
        onMounted(() => {
            // Enforce default payer "Me" if empty
            if (!props.form.payer) props.form.payer = '我';
        });
    ...
    watch: {
        'form.payer': {
            handler(val) {
                // If payer is empty/null/undefined, set to '我'
                // This handles initial load and any accidental clearing
                if (!val) {
                    this.form.payer = '我';
                }
            },
    ...
    computed: {
        activeCurrencyCodes() {
            // Use activeCurrencies from config if available, otherwise fallback to effectiveRates keys
            if (this.systemConfig && this.systemConfig.activeCurrencies && this.systemConfig.activeCurrencies.length > 0) {
                return this.systemConfig.activeCurrencies;
            }
            const extras = this.effectiveRates ? Object.keys(this.effectiveRates) : [];
            const set = new Set([this.baseCurrency.value, ...extras]);
        ...
        showPreview() {
            return this.form.amount > 0 && this.form.currency !== this.baseCurrency;
        },
        previewAmount() {
            if (!this.showPreview) return '';
            const converted = convert(Number(this.form.amount), this.form.currency, this.baseCurrency.value, this.effectiveRates || {});
            return formatCurrency(converted, this.baseCurrency.value);
        }
    },
    watch: {
        'form.spendDate': {
    ...
    methods: {
        ...
        prepareAndSubmit() {
            ...
            // Explicitly ensure currency is passed (it's in form.currency but good to be explicit mentally)
            this.$emit('submit');
        }
    }
};
