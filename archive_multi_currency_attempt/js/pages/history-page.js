...
export const HistoryPage = {
        ...
        groupedTransactions() {
            ...
            return Object.keys(groups).sort((a, b) => b.localeCompare(a)).map(key => ({
                ...
                items: groups[key].sort((a, b) => {
                    const dateA = a.spendDate || '';
                    const dateB = b.spendDate || '';
    ...
    methods: {
        ...
        getPaymentIcon(id) {
            if (!this.paymentMethods) return 'payments';
            const pm = this.paymentMethods.find(p => p.id === id);
            return pm ? (pm.icon || 'payments') : 'payments';
        },
        getProjectName(id) {
        ...
        getIcon(id) {
            if (!this.categories) return 'payments';
