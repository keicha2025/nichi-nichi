...
export const SettingsPage = {
    ...
    template: `
    <section class="space-y-4 pt-2 animate-in fade-in pb-24">
        <!-- 0. 基本設定卡片 -->
        <!-- 0. Currency & System Management -->
        <div class="bg-white p-6 rounded-[2rem] muji-shadow border border-gray-50 space-y-6">
            <h3 class="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-medium px-2">Currency & System</h3>
            
            <!-- Tier 1: Primary Display Currency (Imitating Stats Page Project Selector) -->
            <div class="flex flex-col space-y-2 px-2">
                <span class="text-[10px] text-gray-400 uppercase tracking-widest font-medium">主要顯示幣別</span>
                <app-select 
                    v-model="localConfig.baseCurrency" 
            ...
            <!-- Tier 2: Currency Management Entry -->
            <div class="px-2">
                <button @click="$emit('open-currency-edit')" class="w-full bg-gray-50 hover:bg-gray-100 p-4 rounded-2xl flex items-center justify-between group transition-all border border-gray-100">
                    <div class="flex items-center space-x-3">
                        <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-gray-500 shadow-sm border border-gray-100 group-hover:scale-110 transition-transform">
                            <span class="material-symbols-rounded">currency_exchange</span>
                        </div>
                        <div class="flex flex-col items-start">
                            <span class="text-xs font-bold text-gray-700">匯率與幣別管理</span>
                            <span class="text-[10px] text-gray-400">設定顯示幣別與匯率</span>
                        </div>
                    </div>
                    <span class="material-symbols-rounded text-gray-300 text-sm group-hover:translate-x-1 transition-transform">arrow_forward_ios</span>
                </button>
            </div>
            ...
            <!-- Save Button -->
            <button @click="saveSettings" :disabled="saving" class="w-full bg-[#4A4A4A] text-white py-4 rounded-2xl text-[10px] font-medium tracking-[0.3em] uppercase active:scale-95 transition-all shadow-lg mt-2">
                {{ saving ? 'Saving...' : '更新設定' }}
            </button>
             <!-- Clear Guest Data -->
             <div v-if="appMode === 'GUEST'" class="pt-2 text-center">
                <button @click="$emit('clear-guest-data')" class="text-red-300 hover:text-red-500 text-[10px] tracking-widest py-2 transition-colors">
                    清除訪客資料
                </button>
            </div>
        </div>
    ...
    emits: ['update-config', 'update-user-data', 'view-project', 'view-friend', 'login', 'logout', 'clear-guest-data', 'create-project', 'open-icon-edit', 'open-currency-edit', 'clear-account-data'],
    data() {
        return {
            localConfig: {
                user_name: '',
    ...
    computed: {
        ...
        baseCurrencyOptions() {
            if (!this.localConfig.activeCurrencies) return [];
            return this.localConfig.activeCurrencies.map(code => ({
                label: code,
                value: code
            }));
        },
        ...
        activeRateList() {
            ...
            return this.allCurrencies.filter(c =>
                this.localConfig.activeCurrencies.includes(c.code) &&
                c.code !== this.localConfig.baseCurrency
            );
        },
        expenseCategories() {
    ...
    methods: {
        async saveSettings() {
            ...
            try {
                ...
                if (!this.localConfig.user_name) this.localConfig.user_name = '';
    ...
    mounted() {
        ...
        // Valid defaults initialization
        if (!this.localConfig.baseCurrency) this.localConfig.baseCurrency = 'JPY';
        if (!this.localConfig.activeCurrencies || this.localConfig.activeCurrencies.length === 0) {
            this.localConfig.activeCurrencies = ['JPY', 'TWD'];
        }
        if (!this.localConfig.exchangeRates || Object.keys(this.localConfig.exchangeRates).length === 0) {
            this.localConfig.exchangeRates = { 'TWD': this.localConfig.fx_rate || 0.22 };
        }
    }
};
